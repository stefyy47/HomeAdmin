...
const getUserDataLoaders = dbInstance => {
  return {
    userById: new DataLoader(ids =>
      dbInstance
        .select('Id', 'ExternalId', 'UserName', 'FirstName', 'LastName')
        .from('FamilyMember')
        .whereIn('Id', ids)
    ...
    userByExternalId: new DataLoader(externalIds =>
      dbInstance
        ...
        .then(rows =>
          externalIds.map(externalId => rows.find(row => row.externalId.toUpperCase() === externalId.toUpperCase()))
        )
    ),
    userRightsByUserId: new DataLoader(userIds =>
      dbInstance
        .select('Id', 'UserId', 'RightId')
        .from('UserRight')
