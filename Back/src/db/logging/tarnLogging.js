...
module.exports.initializeTarnLogging = pool => {
  ...
  pool.on('acquireRequest', eventId => console.log(`acquireRequest: ${eventId}`.yellow))
  pool.on('acquireSuccess', (eventId, resource) => console.log(`acquireSuccess: ${message(eventId, resource)}`.yellow))
  pool.on('acquireFail', (eventId, err) => console.log(`acquireFail: ${eventId} error: ${err}`.red))
  ...
  pool.on('createRequest', eventId => console.log(`createRequest: ${eventId}`.yellow))
  pool.on('createSuccess', (eventId, resource) => console.log(`createSuccess: ${message(eventId, resource)}`.yellow))
  pool.on('createFail', (eventId, err) => console.log(`createFail: ${eventId} error: ${err}`.red))

  // resource is destroyed and evicted from pool
  // resource may or may not be invalid when destroySuccess / destroyFail is called
  pool.on('destroyRequest', (eventId, resource) => console.log(`destroyRequest: ${message(eventId, resource)}`.yellow))
  pool.on('destroySuccess', (eventId, resource) => console.log(`destroySuccess: ${message(eventId, resource)}`.yellow))
