...
const shouldSkipLogging = (operationName, logLevel) => {
  ...
  switch (logLevel) {
    case INFO:
      return APOLLO_LOGGING_LEVEL === loggingLevels.INFO || APOLLO_LOGGING_LEVEL === loggingLevels.DEBUG
    case DEBUG:
      return APOLLO_LOGGING_LEVEL === loggingLevels.DEBUG
    case ERROR:
      return APOLLO_LOGGING_LEVEL === loggingLevels.ERROR || APOLLO_LOGGING_LEVEL === loggingLevels.DEBUG
  }
}
...
const initializeDbLogging = (context, operationName) => ({
  logInfo: (message, code, autoSave = false) =>
    ...
    logEvent(context, message, code, loggingLevels.INFO, null, autoSave),
  logDebug: (message, code, autoSave = false) =>
    shouldSkipLogging(operationName, loggingLevels.DEBUG) &&
    logEvent(context, message, code, loggingLevels.DEBUG, null, autoSave),
  logError: (message, code, error) =>
    shouldSkipLogging(operationName, loggingLevels.ERROR) && logDbError(context, message, code, loggingLevels.ERROR, error)
})
